# Вставьте сюда ВЕСЬ текущий код из db.py до функции _init_postgres_schema
import os
import json
import asyncio
import importlib
...
async def _init_postgres_schema() -> None:
    ...
    async with pool.acquire() as connection:
        ...
        await connection.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_accounts_mode 
            ON accounts (mode)
            """
        )
        ...
        await connection.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_comment_logs_created 
            ON comment_logs (created_at)
            """
        )
...
async def db_update_warmup_schedule(
    ...
    *,
    ...
    if warmup_end is not None:
        updates.append("warmup_end_at = ?")
        params.append(warmup_end.isoformat())
...
# Вставьте сюда остальной код из db.py после функции _init_postgres_schema
